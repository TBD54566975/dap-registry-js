...
export class Crypto {
  ...
  static async verify({ jws, detachedPayload }: VerifyOptions) {
    ...
    if (length !== 3) {
      throw new InvalidJws('Signature verification failed: Expected valid JWS with 3 parts');
    }
    ...
    if (!jwsHeader.alg || typeof jwsHeader.alg !== 'string') {
      throw new InvalidJws('Signature verification failed: Missing or invalid algorithm in JWS header');
    }
    if (!jwsHeader.kid || typeof jwsHeader.kid !== 'string') {
      throw new InvalidJws('Signature verification failed: Missing or invalid key ID in JWS header');
    }
